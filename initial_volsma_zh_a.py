...
def calc_volume_ratio(stock_code):
    # 获取历史数据（含成交量）
    df = ak.stock_zh_a_hist(symbol=stock_code, period="daily", start_date="20250707",
                                                end_date='20250715', adjust="")
    ...
    data=
    df = df.rename(columns={'代码': 'code', '名称': 'name', 'date': 'date', 'volume_ratio': 'current','':'trade_volume', '5d_avg_vol'})
...
if __name__ == '__main__':
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM selected_zh_a_up2to5")  # 执行查询:ml-citation{ref="10" data="citationList"}
